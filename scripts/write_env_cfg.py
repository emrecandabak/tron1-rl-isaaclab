...
class CommandsCfg:
    ...
    gait_command = mdp.UniformGaitCommandCfg(
        ...
        ranges=mdp.UniformGaitCommandCfg.Ranges(
            frequencies=(1.3, 1.8),       # Allows walking (1.3 Hz) to fast running cadence (1.8 Hz)
            offsets=(0.5, 0.5),           # Strictly alternating bipedal gait (180 deg out of phase)
            durations=(0.40, 0.50),       # 0.40-0.50 duty factor: allows flight phase (<0.5) for running
            swing_height=(0.12, 0.18),    # High foot clearance (12-18 cm) for clear, non-shuffling steps
        ),
    )
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-0.6, 1.2), lin_vel_y=(-0.25, 0.25), ang_vel_z=(-1.0, 1.0), heading=(-math.pi, math.pi)
        ),
    )
...
class RewardsCfg:
    ...
    rew_feet_air_time = RewTerm(
        ...
        params={
            ...
            "threshold": 0.25,  # Requires >=0.25s swing air time on touchdown (forces big strides)
    ...
    pen_lin_vel_z = RewTerm(
        ...
        weight=-2.0,  # Softened from -10 to allow natural vertical running flight dynamics
    )
    ...
    # RELAXED pitch control (allows robot to pitch slightly forward during high-speed acceleration)
...
@configclass
class CurriculumCfg:
    """Curriculum terms for the MDP (Flat ground velocity progression)"""

    # Flat ground velocity command curriculum: smoothly expands velocity commands from walking (0.6-1.2 m/s) to fast running (up to 2.5 m/s)
    velocity_command_curriculum = CurrTerm(
        func=mdp.velocity_command_curriculum,
        params={
            "command_name": "base_velocity",
            "reward_threshold": 0.70,
            "max_lin_vel_x": (-1.0, 2.5),
            "max_lin_vel_y": (-0.4, 0.4),
            "step_size_x": 0.05,
            "step_size_y": 0.02,
        },
    )
...
target_path = r'D:\Downloads\tron1-rl-isaaclab\exts\bipedal_locomotion\bipedal_locomotion\tasks\locomotion\cfg\SF\limx_base_env_cfg.py'
